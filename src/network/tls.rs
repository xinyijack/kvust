...
impl TlsClientConnector {
    ...
    where S: Unpin + AsyncRead + AsyncWrite + Send,
    {
        let dns = DNSNameRef::try_from_ascii_str(self.domain.as_str())
            .map_err(|_| {
                KvError::Internal("Invalid DNS name".into())
            })?;
...
impl TlsServerAcceptor {
    ...
    pub fn new(cert: &str, key: &str, client_ca: Option<&str>) -> Result<Self, KvError> {
        ...
        config
            ...
            .map_err(|_| {KvError::CertifcateParseError("CA", "server")})?;
...
pub mod tls_utils {
    ...
    pub fn tls_connector(client_cert: bool) -> std::result::Result<TlsClientConnector, KvError> {
        let ca = Some(CA_CERT);
    ...
    pub fn tls_acceptor(client_cert: bool) -> std::result::Result<TlsServerAcceptor, KvError> {
        let ca = Some(CA_CERT);
        match client_cert {
